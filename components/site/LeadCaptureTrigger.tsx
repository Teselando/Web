...
export function LeadCaptureTrigger({ className, label, targetId }: LeadCaptureTriggerProps) {
  if (targetId) {
    return <PrimaryButton className={className} href={`#${targetId}`} onClick={() => { track("cta_click"); requestAnimationFrame(() => document.querySelector<HTMLElement>(`#${targetId} [data-lead-capture-phone]`)?.focus({ preventScroll: true })); }}>{label}</PrimaryButton>;
  }
